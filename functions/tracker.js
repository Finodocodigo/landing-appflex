import { productKeyFromUrl, resolveProductPixel } from './_pixels.js';
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // --- Source gate for InitiateCheckout → Meta CAPI ---
    // The offer runs Google Ads alongside Meta. A Google-sourced visitor still
    // gets a synthetic _fbp from the edge middleware, so fbp presence alone
    // does NOT prove Meta attribution. Sending these checkout-intent events to
    // Meta only dilutes the event's identifier coverage (fbp/fbc/external_id
    // EMQ) without giving Meta anything it can attribute — their Purchase is
    // reported to Google Ads via the webhook, not to Meta.
    //
    // We suppress the Meta fire (GA4 still receives it) when the click carries a
    // Google identifier and no genuine Meta click (fbc). Scoped to
    // InitiateCheckout: Lead/Purchase carry hashed PII and must always flow.
    const eventNameLower = (body.event_name || '').toLowerCase();
    const hasMetaClick = !!fbc;
    const hasGoogleClick = !!(sessionData.gclid || sessionData.gbraid || sessionData.wbraid);
    const suppressMetaForSource =
    ...
    const productKey = productKeyFromUrl(body.event_source_url);
...
// META CAPI
// -------------------------------------------------------
async function sendToMeta({ body, clientIp, userAgent, fbp, fbc, hashedEm, hashedFn, hashedLn, hashedPh, hashedExternalId, sessionData, pixelId, accessToken, env }) {
  ...
  // custom_data is required by Meta for Lead/InitiateCheckout/Purchase quality.
  // For Lead we pass value/currency/content_ids/content_name/content_category.
