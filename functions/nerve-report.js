/**
 * Protection layer for the /nerve-report landing (Cloudflare Pages Function).
 * Nervaline (neuropathy). One of the two protected money pages on this domain.
 *
 ...
 * /nerve-report) and decides what each visitor receives:
 *
...
export async function onRequest(ctx) {
  ...
  const pixelId = env && env.META_PIXEL_ID_NERVE;
  if (pixelId) {
    const html = (await res.text()).split("__REPLACE_META_PIXEL_ID_NERVE__").join(pixelId);
