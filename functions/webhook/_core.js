...
//     productId:       string,
//     productName:     string,
//     productCodename: string,   // optional; platform product code (BuyGoods),
//                                // drives the Meta CAPI codename filter
...
import PRODUCTS_CONFIG, {
  META_PRODUCT_CODENAME_ALLOWLIST,
  META_EMPTY_ALLOWLIST_BEHAVIOR,
  META_PIXEL_BY_CODENAME,
...
// -----------------------------------------------------------------------------
// Meta CAPI product-codename gate.
//
// Decides whether a purchase may fire the Meta `Purchase` event, based on
// config/products.js → META_PRODUCT_CODENAME_ALLOWLIST. Config-driven (same
// per-platform-map shape as PRODUCTS_CONFIG), NOT platform branching:
//
//   - platform absent from the map   → no filter (always allowed). Platforms
//     without a {PRODUCT_CODENAME} concept (Eduzz/Hotmart/Kiwify) live here.
//   - platform with a NON-EMPTY list → allowed only if the purchase's codename
//     STARTS WITH an entry (case-insensitive, trimmed). Entries are product-name
//     prefixes: BuyGoods codenames are <product><bottles> (e.g. 'neu6', 'neu3'),
//     so 'neu' matches every NeuroMindPro bottle tier.
//   - platform with an EMPTY list    → governed by META_EMPTY_ALLOWLIST_BEHAVIOR
//     ('allow' = fire for all, 'block' = fire for none) until the list is filled.
//
// Only Meta is gated here (the user's filter is Meta-specific); GA4 / Google Ads
// keep their own per-product config and are unaffected.
// -----------------------------------------------------------------------------
export function isMetaAllowed(platform, productCodename) {
  const list = META_PRODUCT_CODENAME_ALLOWLIST?.[platform];
  if (!Array.isArray(list)) return true;            // platform not gated
  if (list.length === 0) return META_EMPTY_ALLOWLIST_BEHAVIOR !== 'block';
  const cn = String(productCodename || '').trim().toLowerCase();
  if (!cn) return false;                            // gated, but sale carried no codename
  return list.some(prefix => {
    const p = String(prefix).trim().toLowerCase();
    return p && cn.startsWith(p);                   // product-name prefix match
  });
}

// -----------------------------------------------------------------------------
// Meta CAPI per-product pixel resolver (act08 = 2 produtos / 2 pixels).
//
// Routes a purchase to the right Meta pixel + CAPI token based on the
// {PRODUCT_CODENAME} prefix, using config/products.js → META_PIXEL_BY_CODENAME.
// Same prefix-match style as isMetaAllowed. Returns the resolved IDs read from
// `env` (pixel ID + token are kept as Cloudflare env vars/secrets, never in the
// repo). Falls back to the generic env.META_PIXEL_ID / env.META_ACCESS_TOKEN
// when no prefix matches (e.g. single-pixel setups, or placeholders not yet
// replaced) so the stack still works without per-product config.
// -----------------------------------------------------------------------------
export function resolveMetaPixel(platform, productCodename, env) {
  const map = META_PIXEL_BY_CODENAME?.[platform];
  const cn = String(productCodename || '').trim().toLowerCase();
  if (map && cn) {
    for (const [prefix, cfg] of Object.entries(map)) {
      const p = String(prefix).trim().toLowerCase();
      if (p && cn.startsWith(p)) {
        const pixelId = env?.[cfg.pixelEnvKey] || env?.META_PIXEL_ID || '';
        const accessToken = env?.[cfg.tokenEnvKey] || env?.META_ACCESS_TOKEN || '';
        return { pixelId, accessToken };
      }
    }
  }
  return { pixelId: env?.META_PIXEL_ID || '', accessToken: env?.META_ACCESS_TOKEN || '' };
}
...
export async function processPurchase({ parsed, env, context }) {
  ...
  // Fire server-side tracking when we have an attributed session (subid matched
  // a checkout_sessions row) OR a strong identifier on the purchase itself
  // (hashed/raw email). The latter lets a BuyGoods postback carrying {EMAILHASH}
  // still convert even if the DTC page dropped the subid and there is no session.
  if ((parsed.subid && checkoutData.subid) || parsed.emailHash || parsed.email) {
...
async function handleTracking({ parsed, eventId, eventTime, env }) {
  ...
  const [metaResult, ga4Result, googleAdsResult] = await Promise.allSettled([
    sendToMeta({ checkoutData, hashedEm, hashedFn, hashedLn, hashedPh, hashedExternalId, eventId, eventTime, value, currency, productName, contents, metaAllowed, platform, productCodename, env }),
...
async function handlePurchaseLog({ parsed, eventId, eventTime, resultMap, env }) {
  ...
  const { subid, email, name, phone, value, currency, transactionId, productId, productName, productCodename, checkoutData, platformUtm, items, productConfig } = parsed;
  const tracking = resultMap.tracking || {};
  const encharge = resultMap.encharge || {};
  const manychat = resultMap.manychat || {};

  // Revenue classification for the dashboard split (Initial / Upsell /
  // Backend / Recurring). Unconfigured products default to 'initial'.
  const allowedRevenueTypes = new Set(['initial', 'upsell', 'backend', 'recurring']);
  const configuredType = productConfig?.revenueType;
  const revenueType = allowedRevenueTypes.has(configuredType) ? configuredType : 'initial';
  ...
  try {
    const result = await env.DB.prepare(`
      INSERT INTO purchase_log (
        ...
        product_id, product_name, product_codename,
    ...
    `).bind(
      ...
      productId || '', productName || '', productCodename || '',
...
// -----------------------------------------------------------------------------
async function sendToMeta({ checkoutData, hashedEm, hashedFn, hashedLn, hashedPh, hashedExternalId, eventId, eventTime, value, currency, productName, contents, metaAllowed, platform, productCodename, env }) {
  // Product-codename filter (config/products.js). Skipped sales still hit
  // purchase_log with this reason in meta_response_body, so they're auditable
  // and replayable once the codename is allowlisted.
  if (metaAllowed === false) {
    return { skipped: 'product_codename not in Meta allowlist', payload: null, response: null };
  }
  // Per-product pixel routing (act08 = 2 produtos / 2 pixels): resolve the
  // pixel ID + CAPI token from the {PRODUCT_CODENAME} prefix. Falls back to the
  // generic META_PIXEL_ID / META_ACCESS_TOKEN when no prefix matches.
  const { pixelId, accessToken } = resolveMetaPixel(platform, productCodename, env);
