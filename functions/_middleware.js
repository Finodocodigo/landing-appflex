import { BLOCK, META } from './_bots.js';

export async function onRequest(context) {
  const { request, next, env } = context;
  const url = new URL(request.url);

  // Crawlers get no tracking cookies and no `sessions` row. The cloak
  // Functions (breath-report.js / nerve-report.js) still decide what each
  // crawler is served; here we just hand control downstream untouched so bot
  // traffic never pollutes attribution data. Uses the same regexes as the cloak.
  const ua = request.headers.get('user-agent') || '';
  if (BLOCK.test(ua) || META.test(ua)) {
    return next();
  }
